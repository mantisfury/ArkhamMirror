...
def keyboard_shortcuts_listener() -> rx.Component:
    ...
    (function() {
        ...
        document.addEventListener('keydown', function(event) {
            ...
            // ACH Matrix shortcuts (1-5 for ratings, Arrow keys for navigation)
            // Only active when an ACH matrix cell has focus
            const achFocused = document.querySelector('[data-ach-matrix-active="true"]');
            if (achFocused) {
                // 1-5 keys for quick rating
                if (['1', '2', '3', '4', '5'].includes(event.key)) {
                    event.preventDefault();
                    const ratingTrigger = document.querySelector('[data-ach-rating="' + event.key + '"]');
                    if (ratingTrigger) {
                        ratingTrigger.click();
                    }
                }
                ...
                if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Tab'].includes(event.key)) {
                    ...
                    const navTrigger = document.querySelector('[data-ach-nav="' + directionMap[event.key] + '"]');
                    if (navTrigger) {
                        navTrigger.click();
                    }
                }
