...
class ACHAnalysis(Base):
    """
    An ACH analysis session following Heuer's 8-step methodology.
    Each analysis has a focus question and contains multiple hypotheses and evidence items.
    """

    __tablename__ = "ach_analyses"
    id = Column(Integer, primary_key=True, autoincrement=True)
    project_id = Column(Integer, ForeignKey("projects.id"), nullable=True)
    title = Column(String, nullable=False)  # Short title for the analysis
    focus_question = Column(Text, nullable=False)  # The central question being analyzed
    description = Column(Text, nullable=True)  # Optional longer description
    status = Column(String, default="draft")  # draft, in_progress, complete, archived

    # Step 7: Sensitivity Analysis
    sensitivity_notes = Column(Text, nullable=True)  # User's manual notes
    key_assumptions = Column(Text, nullable=True)  # JSON list of assumptions

    # Methodology tracking
    current_step = Column(Integer, default=1)  # 1-8 (Heuer's 8 steps)
    steps_completed = Column(Text, default="[]")  # JSON list of completed step numbers
...
class ACHHypothesis(Base):
    """
    A competing hypothesis within an ACH analysis.
    Each hypothesis represents one possible explanation for the focus question.
    """

    __tablename__ = "ach_hypotheses"
    id = Column(Integer, primary_key=True, autoincrement=True)
    analysis_id = Column(
        Integer, ForeignKey("ach_analyses.id", ondelete="CASCADE"), nullable=False
    )
    label = Column(String, nullable=False)  # Short label (e.g., "H1", "H2")
    description = Column(Text, nullable=False)  # Full hypothesis text
    display_order = Column(Integer, default=0)  # For UI ordering
    color = Column(String, default="#3b82f6")  # Hex color for charts

    # Step 6: Scoring (calculated, but cached for performance)
    inconsistency_score = Column(Float, default=0.0)  # Lower is better

    # Step 8: Future Indicators / Milestones
    future_indicators = Column(Text, nullable=True)  # What we'd expect to see if true
    indicator_timeframe = Column(String, nullable=True)  # "weeks", "months", "years"

    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)


class ACHEvidence(Base):
    """
    A piece of evidence or argument used in an ACH analysis.
    Each evidence item is rated against all hypotheses.
    """

    __tablename__ = "ach_evidence"
    id = Column(Integer, primary_key=True, autoincrement=True)
    analysis_id = Column(
        Integer, ForeignKey("ach_analyses.id", ondelete="CASCADE"), nullable=False
    )
    label = Column(String, nullable=False)  # Short label (e.g., "E1", "E2")
    description = Column(Text, nullable=False)  # The evidence text
    display_order = Column(Integer, default=0)  # For UI ordering

    # Evidence metadata
    evidence_type = Column(
        String, default="fact"
    )  # fact, testimony, document, assumption, argument
    reliability = Column(String, default="medium")  # high, medium, low
    source = Column(Text, nullable=True)  # Source text/quote
    source_document_id = Column(
        Integer, ForeignKey("documents.id"), nullable=True
    )  # Link to corpus

    # Step 4: Diagnosticity (calculated, but cached)
    diagnosticity_score = Column(Float, default=0.0)  # Higher = more diagnostic
    is_critical = Column(Integer, default=0)  # 1 = marked as critical for sensitivity

    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)


class ACHRating(Base):
    """
    A single cell in the ACH matrix: rating of one evidence item against one hypothesis.
    Rating scale: CC (very consistent), C (consistent), N (neutral),
    ...
    hypothesis_id = Column(
        Integer, ForeignKey("ach_hypotheses.id", ondelete="CASCADE"), nullable=False
    )
    evidence_id = Column(
        Integer, ForeignKey("ach_evidence.id", ondelete="CASCADE"), nullable=False
    )

    # Rating value: CC, C, N, I, II, or empty string (unrated)
    rating = Column(String, default="")
    notes = Column(Text, nullable=True)  # Optional notes for this cell
    ...
    __table_args__ = (
        UniqueConstraint(
            "hypothesis_id", "evidence_id", name="uq_ach_rating_hyp_evidence"
        ),
    )


class ACHMilestone(Base):
    """
    Future indicator tracking for Step 8 of ACH methodology.
    Allows tracking of expected outcomes over time.
    """

    __tablename__ = "ach_milestones"
    id = Column(Integer, primary_key=True, autoincrement=True)
    analysis_id = Column(
        Integer, ForeignKey("ach_analyses.id", ondelete="CASCADE"), nullable=False
    )
    ...
    description = Column(Text, nullable=False)  # What to watch for
    expected_by = Column(DateTime, nullable=True)  # When to check

    # Tracking status
    observed = Column(Integer, default=0)  # 0=pending, 1=observed, -1=contradicted
...
class ACHAnalysisSnapshot(Base):
    """
    Snapshot of an ACH analysis at a specific point in time.
    Used for version history and comparison.
    """

    __tablename__ = "ach_analysis_snapshots"
    id = Column(Integer, primary_key=True, autoincrement=True)
    analysis_id = Column(
        Integer, ForeignKey("ach_analyses.id", ondelete="CASCADE"), nullable=False
    )
    ...
    label = Column(
        String, nullable=True
    )  # e.g., "Initial Hypotheses", "Post-Evidence", "Final V1"
    description = Column(Text, nullable=True)  # User notes on this version

    # The actual data - stored as JSON
    # This captures hypotheses, evidence, ratings, and scores at this moment
