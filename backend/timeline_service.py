...
from backend.llm_service import chat_with_llm
...
def extract_date_mentions(text: str, context_chars: int = 50) -> List[Dict]:
    ...
    pattern5 = r'\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4}\b'
    ...
    all_patterns = [
        (pattern1, "explicit"), (pattern2, "explicit"),
        (pattern3, "explicit"), (pattern4, "explicit"),
        (pattern5, "explicit"), (pattern6, "explicit"),
        (pattern7, "explicit"), (pattern8, "relative")
    ]
...
JSON output:"""
    ...
    try:
        response = chat_with_llm(prompt, temperature=0.3, max_tokens=1500)
...
def determine_date_precision(date_text: str) -> str:
    ...
    # Month and year
    if re.match(r'^(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+\d{4}$', date_text, re.IGNORECASE):
        return "month"
...
def extract_timeline_from_chunk(chunk_text: str, chunk_id: int, doc_id: int) -> Tuple[List[Dict], List[Dict]]:
    """
...
def analyze_timeline_gaps(events: List[Dict], gap_threshold_days: int = 30) -> List[Dict]:
    """
